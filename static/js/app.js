...
PP.App = (function() {
    ...
    var model = (function() {
        ...
        return {
            ...
            initialize : function(fcts,cats) {
                factors = fcts;
                var factorsMap = 
                    _.object(_.map(factors, function(factor) {
                        return [factor.id, factor]
                    }));
                    ...
                    _.map(cats, function(category) {
                        category.layers = 
                            _.map(category.layers, function(layer) {
                                return factorsMap[layer];
                            });
                        return category;
                    });

                // Set defalut weights onto the layers
                _.each(factors, function(factors) {
                    factor.weight = weights;
                });
            ...
            getFactors: function() { return layers; },

            addActiveFactor: function(layer) { addActiveFactors([layer]); }

            addActiveFactors: function(layers) {
                if(!_.contains(activeFactors,layer)) {
                    activeFactors.push(layer);
                    inactiveFactors.push(layer);
                    notifyChange();
                };
            },
            removeActiveFactor: function(layer) {
                if(_.contains(activeFactors,layer)) {
                    var i = activeFactors.indexOf(layer);
                    activeFactors.splice(i,1);
                    inactiveFactors.splice(i,1);
                    notifyChange();
                };
            },
            toggleActiveFactor: function(layer) {
                activeFactors.length = 0;
                activeFactors.push(layer);
                notifyChange();
            },

            resetActiveFactor: function(layer) {
                $.extend(true, activeFactors, inactiveFactors);
                notifyChange();
            },

            updateFactorWeight: function(layer,weight) {
            ...
            getActiveFactorWeights: function() {
                var activeWeights = {};
                _.forEach(activeFactors, function(id) {
    ...
    var weightedOverlay = (function() {
        ...
        var getLayerStrings = function() {
            var layers = model.getActiveFactorWeights();
            var ls = [];
            var ws = [];
    ...
    var factors = (function() {
        ...
        var loadFactors = function() {
            ...
            _.forEach(model.getFactors(), function(factor) {
                // Is there a better way to create the templates so that I can bind
                // the on('slide',...) event to close over the specific 'factor' var it is for?
                var $parentContainer = $factorsList.append(factorTemplate(factor));
                var $container = $parentContainer.find('#factor-'+factor.id);

                $container.find('.factor-info').tooltip({ placement:'left', container:'#sidebar' });

                $container.find('.css-radio').on('change', function(e) {
                    model.toggleActiveFactor(factor);
                    toggleFactorRadio(e);
                });

                $sidebar.find('#all-radio').on('change', function(e) {
                    model.resetActiveFactor(factor);
                    toggleFactorRadio(e);
                });

                $container.find('.slider').slider().on('slide', function(e) {
                    factor.weight = e.value;
                    updateFactorWeight(e);
                });
                $container.find('.factor-remove').on('click', function(e) {
                    model.removeActiveFactor(factor);
                    removeFactor(e);
                });
            });
        };
        ...
        var loadScenarios = function(scenarios) {
            var $scenario_select = $('#scenario-select');
            
            var indexedScenarios = { }

            _.each(scenarios, function(scenario) {
                var option = $('<option value="' + scenario.name +'">' + scenario.name + '</option>');
                $scenario_select.append(option);
                indexedScenarios[scenario.name] = scenario;
            });

            $scenario_select.on('change', function() {
                var scenario = indexedScenarios[$(this).val()];
                if(scenario.layers == "all") {
    ...
    var init = function () {
        $.when(
            ...
            function(factorsJson, categoriesJson, scenariosJson, geoserverLayersJson) {
                model.initialize(factorsJson[0].factors,categoriesJson[0].categories);
                toolLegend.init();
